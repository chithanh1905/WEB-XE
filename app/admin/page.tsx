...
type Tab = "khach-hang" | "bang-gia" | "anh-xe" | "banner";

const TABS: { id: Tab; label: string; icon: string }[] = [
  { id: "khach-hang", label: "Khách hàng", icon: "👥" },
  { id: "bang-gia", label: "Bảng giá", icon: "💰" },
  { id: "anh-xe", label: "Ảnh xe", icon: "🚗" },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>("khach-hang");
  ...
  const filtered = data.filter((r) => {
    ...
    const matchSearch = !q || r.ho_ten?.toLowerCase().includes(q) || r.so_dien_thoai?.includes(q) || r.dia_chi?.toLowerCase().includes(q);
  ...
  const exportCSV = () => {
    ...
    const rows = filtered.map((r) => [r.ho_ten, r.so_dien_thoai, r.dia_chi || "", r.xe_quan_tam || "", r.ghi_chu || "", r.created_at ? new Date(r.created_at).toLocaleString("vi-VN") : ""]);
    const csv = [header, ...rows].map((r) => r.map((c) => `"${c}"`).join(",")).join("\n");
    const blob = new Blob(["﻿" + csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `khach-hang-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-500 text-sm mt-1">Quản lý nội dung & khách hàng</p>
          </div>
          <div className="space-y-3">
            <input type="password" placeholder="Nhập mật khẩu..." value={pass}
            ...
            <button onClick={login} className="w-full py-3 rounded-xl text-white font-bold text-base" style={{ background: "var(--vf-blue)" }}>
              Đăng nhập
            </button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-black text-gray-900 text-base">Quản trị nội dung</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setAuthed(false); setPass(""); setData([]); }}
            className="text-sm px-3 py-2 rounded-lg border border-gray-200 hover:bg-gray-50 text-gray-500 font-semibold">
            Đăng xuất
          </button>
        </div>
      </div>
        ...
        <div className="flex gap-0">
          {TABS.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-2 px-5 py-3.5 text-sm font-bold border-b-2 transition-colors -mb-px ${activeTab === tab.id ? "border-blue-600 text-blue-700" : "border-transparent text-gray-500 hover:text-gray-800"}`}>
              <span>{tab.icon}</span>
              {tab.label}
              {tab.id === "khach-hang" && data.length > 0 && (
                <span className="ml-1 bg-blue-100 text-blue-700 text-xs font-black px-1.5 py-0.5 rounded-full">{data.length}</span>
              )}
            </button>
          ))}
        </div>
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              {[
                ...
                { label: "Tuần này", value: data.filter((r) => { if (!r.created_at) return false; return (new Date().getTime() - new Date(r.created_at).getTime()) / 86400000 <= 7; }).length, color: "#9333EA" },
            ...
            <div className="flex gap-3 mb-4 flex-wrap">
              <input type="text" placeholder="Tìm tên, SĐT, địa chỉ..." value={search}
                onChange={(e) => setSearch(e.target.value)} className="form-input flex-1 min-w-[200px] text-sm" />
              <select value={filterXe} onChange={(e) => setFilterXe(e.target.value)} className="form-input text-sm min-w-[160px]">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              {loading ? (
                ...
                <div className="py-20 text-center text-gray-400"><p className="text-4xl mb-3">📋</p><p>Chưa có khách hàng nào</p></div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-100">
                        {["#", "Họ tên", "Số điện thoại", "Địa chỉ", "Xe quan tâm", "Ghi chú", "Thời gian"].map((h) => (
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {filtered.map((r, i) => (
                        <tr key={i} className="hover:bg-blue-50/30 transition-colors">
                          ...
                          <td className="px-4 py-3"><a href={`tel:${r.so_dien_thoai}`} className="font-mono text-blue-600 hover:underline font-semibold">{r.so_dien_thoai}</a></td>
                          <td className="px-4 py-3 text-gray-600 max-w-[160px] truncate">{r.dia_chi || <span className="text-gray-300">—</span>}</td>
                          <td className="px-4 py-3">{r.xe_quan_tam ? <span className="px-2 py-0.5 rounded-full text-xs font-bold text-white" style={{ background: "var(--vf-blue)" }}>{r.xe_quan_tam}</span> : <span className="text-gray-300">—</span>}</td>
                          <td className="px-4 py-3 text-gray-500 max-w-[180px] truncate">{r.ghi_chu || <span className="text-gray-300">—</span>}</td>
                          <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{r.created_at ? new Date(r.created_at).toLocaleString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  <div className="px-4 py-3 border-t border-gray-100 text-xs text-gray-400">Hiển thị {filtered.length} / {data.length} khách hàng</div>
                </div>
              )}
            </div>
          </>
